...
class AutoSalaryCalculator:
    ...
    def calculate_person_count_bonus(self, person_count):
        ...
        # 110-132人，每人100元
        if person_count >= 110:
            count_110_132 = min(person_count, 132) - 110 + 1
            bonus += count_110_132 * 100
        
        # 132人以上，每人200元
        if person_count > 132:
            count_above_132 = person_count - 132
            bonus += count_above_132 * 200
    ...
    def calculate_seasonal_bonus(self, employees, mask_sales):
        ...
        for employee in employees:
            ...
            # 獲取該員工的水光面膜銷售數量
            # 需要根據員工行號找到對應的淨膚師編號
            employee_mask_count = 0
            for therapist_id, count in mask_sales.items():
                # 這裡需要建立員工行號與淨膚師編號的對應關係
                # 假設淨膚師編號就是行號-11 (12->1, 13->2, 14->3, 15->4)
                if str(employee['row'] - 11) == therapist_id:
                    employee_mask_count = count
                    break
            
            # 充值目標達成獎
            charge_bonus, base_charge_bonus, mask_bonus = self.calculate_charge_target_bonus(
                employee['personal_performance'], employee_mask_count
            )
            
            # 將季獎金資訊添加到員工資料
            employee['person_count_bonus'] = person_count_bonus
            employee['charge_target_bonus'] = charge_bonus
            employee['mask_sales_count'] = employee_mask_count
            
            print(f"   {employee['name']}: 人次{employee['person_count']:.0f} → 人次獎金{person_count_bonus:,}元")
            print(f"   {employee['name']}: 業績{employee['personal_performance']:,.0f} + 面膜{employee_mask_count}組 → 充值獎金{charge_bonus:,}元")
    ...
    def calculate_salary(self, employees, team_bonus_per_person, formal_staff_positions):
        ...
        for employee in employees:
            ...
            # 季獎金
            person_count_bonus = employee.get('person_count_bonus', 0)
            charge_target_bonus = employee.get('charge_target_bonus', 0)
            ...
            result = {
                ...
                'mask_sales_count': employee.get('mask_sales_count', 0)
            }
    ...
    def print_results(self, results, total_performance, total_consumption):
        ...
        for i, result in enumerate(results, 1):
            ...
            print(f"   🎭 充值目標獎金: {result['charge_target_bonus']:,} 元 (面膜{result['mask_sales_count']}組)")
            print("   ─────────────────────────────")
